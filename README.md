...
- Router default sizing is 1 vCPU and 1 GB RAM.
...
- The seed ISO contains `user-data`, `meta-data` with a unique `instance-id`/`local-hostname`, and a minimal `network-config` so VyOS cloud-init treats each router as a fresh instance and applies the router config on first boot.
